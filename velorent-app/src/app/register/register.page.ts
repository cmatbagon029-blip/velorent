...
addIcons({ checkmarkCircleOutline, closeCircleOutline, eyeOutline, eyeOffOutline });
  ...
  template: `
    ...
    <ion-content class="ion-padding">
      <div class="success-content">
        ...
        <ion-button expand="block" (click)="goToLogin()">Go to Login</ion-button>
      </div>
    </ion-content>
  `,
  styles: [`
...
@Component({
  ...
  standalone: true,
  imports: [IonicModule, FormsModule, RouterModule, CommonModule, ReactiveFormsModule, TermsModalComponent, SuccessModalComponent],
...
export class RegisterPage {
  ...
  onEmailInput(event: any) {
    this.showEmailRequirements = event.target.value.length > 0;
  }

  onPasswordInput(event: any) {
    this.showPasswordRequirements = event.target.value.length > 0;
  }

  onConfirmPasswordInput(event: any) {
    this.showConfirmPasswordRequirements = event.target.value.length > 0;
  }
  ...
  isEmailValid(): boolean {
    return this.registerForm.get('email')?.valid || false;
  }
  ...
  isPasswordComplexValid(): boolean {
    const password = this.registerForm.get('password')?.value;
    return password && password.length >= 6;
  }
  ...
  async register() {
    ...
    if (this.registerForm.valid) {
      ...
      try {
        ...
        const apiUrl = environment?.apiUrl || 'https://velorent-backend.onrender.com/api';
